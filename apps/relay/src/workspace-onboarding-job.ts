...
export function workspaceOnboardingInstruction(input: {
  ...
  const setupKickoff =
    input.selectedApps.length > 0
      ? `\n- mentions ["setup"], idempotencyKey onboarding-setup-thread: "Hey @Setup, privately help me connect the selected apps: ${apps}. Start with what can be verified safely and only ask me to step in for sign-in, consent, or an unavoidable account choice."`
      : "";
...
- mentions ["brand"], idempotencyKey onboarding-marketer-thread: "Hey @Marketer, use [chief-skill:build-brand-profile] to create a useful working profile from our first-party evidence. Ask one focused question here only if a missing preference materially changes it."
- mentions ["prospector"], idempotencyKey onboarding-prospector-thread: "Hey @Prospector, use [chief-skill:find-buying-signals] to find our first real buying signals from the best available evidence. Ask one focused question here only if it materially changes qualification."
- mentions ["engineer"], idempotencyKey onboarding-engineer-thread: "Hey @Engineer, get oriented and prepare the Engineering workspace. Don't change code or deploy anything yet."${setupKickoff}
