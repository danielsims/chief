...
- Everything you need was collected before this run: source "$HOME/.marketer/secrets.env" for MARKETER_GOOGLE_OAUTH_CLIENT_ID, MARKETER_GOOGLE_OAUTH_CLIENT_SECRET and MARKETER_GOOGLE_ADS_DEVELOPER_TOKEN. Build "$HOME/.marketer/google-oauth-client.json" from the client values if it does not exist (same printf as the Analytics flow).
- Log in exactly once with --client-id-file and the UNION of scopes so existing Analytics access survives:
  gcloud auth application-default login --client-id-file="$HOME/.marketer/google-oauth-client.json" --scopes="https://www.googleapis.com/auth/cloud-platform,https://www.googleapis.com/auth/analytics.readonly,https://www.googleapis.com/auth/adwords"
...
- Auth sequence:
  1. If "$HOME/.config/gcloud/application_default_credentials.json" already exists, check what it reaches first. If it lists Analytics accounts, DO NOT silently reuse it: this machine's Google access may belong to a different person or company than the app account. Ask exactly one short question, then stop: "This Mac already has Google access to <account and property names>. Connect it, or use a different Google account?" On a clear yes, proceed straight to verification and the series report with no further questions. On "different account", run the step 4 login (it re-prompts the account choice).
  2. If "$HOME/.marketer/google-oauth-client.json" is missing, emit this input request IMMEDIATELY, in your first message if possible (the Client ID and secret appear on screen in Google's final dialog, so the user only clicks and pastes):
MARKETER_INPUT_REQUEST {"id":"google-oauth-client","title":"Allow Marketer to read your Google Analytics","reason":"Google needs an app key, created once in your Google Cloud account. Nothing in your Analytics changes; you approve read-only access right after.","steps":[{"text":"Sign in to the Google Cloud **credentials page**.","url":"https://console.cloud.google.com/apis/credentials"},{"text":"Click **Create credentials**, then **OAuth client ID**. Create a new one even if others are listed."},{"text":"Type: **Desktop app**. Name: **Marketer**. Click **Create**."},{"text":"Copy the **Client ID** and **Client secret** into the fields below."}],"fields":[{"key":"clientId","label":"Client ID","type":"text","save":{"envKey":"MARKETER_GOOGLE_OAUTH_CLIENT_ID"}},{"key":"clientSecret","label":"Client secret","type":"secret","save":{"envKey":"MARKETER_GOOGLE_OAUTH_CLIENT_SECRET"}}]}
  ...
  3. After the confirmation message, build the client file without printing the values:
. "$HOME/.marketer/secrets.env" && printf '{"installed":{"client_id":"%s","client_secret":"%s","auth_uri":"https://accounts.google.com/o/oauth2/auth","token_uri":"https://oauth2.googleapis.com/token","redirect_uris":["http://localhost"]}}' "$MARKETER_GOOGLE_OAUTH_CLIENT_ID" "$MARKETER_GOOGLE_OAUTH_CLIENT_SECRET" > "$HOME/.marketer/google-oauth-client.json" && chmod 600 "$HOME/.marketer/google-oauth-client.json"
  4. Log in exactly once:
  gcloud auth application-default login --client-id-file="$HOME/.marketer/google-oauth-client.json" --scopes="https://www.googleapis.com/auth/cloud-platform,https://www.googleapis.com/auth/analytics.readonly"
...
0. Values the user provided before this run are in "$HOME/.marketer/secrets.env"; source it first and never ask for something already there.
