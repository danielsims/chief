...
- Auth sequence:
  1. If "$HOME/.config/gcloud/application_default_credentials.json" already exists, check what it reaches first. If it lists Analytics accounts, DO NOT silently reuse it: this machine's Google access may belong to a different person or company than the app account. Say in one line what you found ("This Mac already has Google access to <account names and properties>.") and ask whether to use it or sign in with a different Google account. Wait for the answer; on "different account", run the login command from step 4 (it re-prompts account choice).
