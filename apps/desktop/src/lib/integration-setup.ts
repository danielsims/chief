...
- Auth sequence:
  1. If "$HOME/.config/gcloud/application_default_credentials.json" already exists, try verification first; credentials may already be in place.
...
- After verification passes, run one more Data API report on the chosen property: metric activeUsers, dimension "date", last 14 days, ordered by date ascending. Put it in the result line as "series" (convert GA's YYYYMMDD dates to YYYY-MM-DD) with "metricLabel":"Active users".
