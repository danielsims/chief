...
export function deployAgentTask(
  ...
  return `Deploy only the ${agent.name} agent from this Chief workspace to Vercel. The Eve project is at ${APP_PATH}, relative to the Chief repository root unless it is absolute. Resolve that directory before running commands there.
...
4. Copy $CHIEF_WORKSPACE_DIR/context.md to workspace-input/context.md when it exists.
...
7. Inspect the generated agent directory. It must contain ${agent.name} as the root instructions, its playbooks under agent/skills, and only its schedules. It must not contain another Chief agent or subagent.
...
10. If EXECUTOR_MCP_URL and EXECUTOR_MCP_TOKEN already exist for a hosted Executor, add them to the Vercel project and redeploy. Never invent them. If they are absent, report that connected Chief tools are not yet available to the deployed agent.
...
CHIEF_SETUP_RESULT {"provider":"${AGENT_DEPLOY_PROVIDER}","status":"connected","url":"<production url>","target":"vercel","agentId":"${agent.id}"}`;
}
