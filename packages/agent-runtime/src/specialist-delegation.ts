...
async function executeSpecialistDelegationAttempt(
  ...
  const specialistInstructions = [
    ...
    input.channelId && input.threadRootId
      ? `You were explicitly invited into #${input.channelId} for this work. The invitation message is ${JSON.stringify(input.threadRootId)}. React naturally when you start, then work without a canned acknowledgement. Keep every meaningful checkpoint, user-action request, browser handoff, and final result in that same thread. Ordinary assistant text remains private working output. Do not post routine tool narration.`
      : undefined,
    activeSkill
      ...
      : input.agentId === "prospector"
        ...
        : input.agentId === "setup"
          ? "Complete only the bounded setup task below. Follow the Setup Integration skill: reuse a verified connection, otherwise use the plugin tools available in this runtime and publish any connection card in the owning conversation or thread. Use Projects for repository access. Open the visible browser only for an unavoidable supported authorization step, and let the user handle sign-in, MFA, account choice, and consent. Never invent a provider flow or claim success before a real read-only verification. Return the verified result or the single exact human action still required."
