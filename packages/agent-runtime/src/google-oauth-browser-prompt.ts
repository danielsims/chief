...
function googleOAuthBrowserInstructions({
  ...
  return [
    `Use browser.snapshot, browser.open, browser.click, browser.fill, browser.select, and browser.press as ordinary agent-browser tools. Prefer snapshot @refs and refresh them after every navigation or material UI change. The Google account the human selected is locked for this attempt${lockedAuthUser ? ` as browser identity authuser=${lockedAuthUser}` : ""}. Chief's host enforces that identity on every Google Cloud and OAuth URL. Never open the account menu, switch to another remembered identity, or try to change authuser. If the account cannot access the intended project, make no changes; return to Google's account chooser for the same Cloud URL and pause for the human to choose again. Before mutating anything in Google Cloud, verify the signed-in identity matches the account the user chose for this setup: read the account name and email from the page and if you are not certain it is the right one, state the account you see and ask the user to confirm or switch before continuing. The account the user believes owns the client's property is authoritative; a personal or default account is not.`,
