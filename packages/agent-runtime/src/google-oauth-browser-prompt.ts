...
function googleOAuthBrowserInstructions({
  ...
  return [
    `Use browser.snapshot, browser.open, browser.click, browser.fill, and browser.press as ordinary agent-browser tools. Prefer snapshot @refs and refresh them after every navigation or material UI change. The Google account the human selected is locked for this attempt${lockedAuthUser ? ` as browser identity authuser=${lockedAuthUser}` : ""}. Chief's host enforces that identity on every Google Cloud and OAuth URL. Never open the account menu, switch to another remembered identity, or try to change authuser. If the account cannot access the intended project, make no changes; return to Google's account chooser for the same Cloud URL and pause for the human to choose again.`,
