...
- After verification succeeds, use Executor to call `tools.chief.org.workspace.agentTools.integrationsMarkConnected` with the canonical provider id, its product category, a useful display name, and the verified account or property id as externalId. This is the source of truth for Chief's integration pages. Never call it before a successful provider API request, and never claim success if it fails.
