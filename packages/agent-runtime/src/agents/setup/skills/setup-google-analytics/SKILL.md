...
description: Connect and verify read-only Google Analytics access for a workspace.
---
...
- Resolve the Analytics recipe with `localTools.setup.list`. Create one attempt with `localTools.setup.start` when this thread has none, otherwise resume its current attempt. Reuse that attempt ID and the current Chief session ID for the rest of the work.
- Call `googleAnalytics.authorize` with that session and attempt. If OAuth client provisioning is required, call `googleOAuth.provisionClient` once.
...
- When the user must sign in, publish one calm handoff in the owning thread, leave the browser open, and return the exact private marker `pending-human-signin`. Waiting for the user is not a failure and must not complete or close the attempt.
