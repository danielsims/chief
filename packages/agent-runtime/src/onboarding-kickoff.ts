...
export function onboardingRecoveryPrompt(
  ...
  return [
    ...
    driver === "remote"
      ...
      : "Read onboarding/getting-started.md from the current working directory and resume every unfinished job recorded there. Launch Brand Researcher and Prospector concurrently and exactly once by issuing the two direct localTools.specialistsDelegate calls together before waiting for either, omitting waitSeconds so both continue in the background. Reuse stable delegation IDs and never start equivalent duplicate specialists. Do not say both are underway until both calls return working or completed. Save the verified brand Markdown with the direct localTools.brandProfileSave tool and retain its visible versioned file. Require Prospector to persist five to eight qualified results with direct source URLs through the direct localTools.prospectsSave tool.",
    driver === "remote"
