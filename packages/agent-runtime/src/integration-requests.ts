...
- When Chief resumes you after Google authentication, you own the visible agent-browser session. Use browser.snapshot, browser.open, browser.click, browser.fill, and browser.press as ordinary browser tools. Prefer the @refs returned by snapshots and re-snapshot after every navigation or material UI change because refs expire. The project in Google's post-login URL is untrusted and is not a user selection. Do not call provisionClient again and do not ask the user to operate Google Cloud.
