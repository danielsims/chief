...
- Once the OAuth client is configured, call googleAnalytics.authorize again. Tell the user this is the final Google step: sign in again if asked and approve read-only Analytics access. Open the returned authorization URL with localTools.browserOpen using this current session ID, then call googleAnalytics.complete with the returned state. Do not ask the user to report completion; the pending tool call observes Google's callback.
